...
const char* DATA_PATH = "C:\\Users\\bkcha\\Desktop\\data_2.dat";
...
void fileRead() {
    FILE* dataFile = NULL;
    if (!(0 == fopen_s(&dataFile, DATA_PATH, "rb+"))) {
        printf("\a\n%s경로의 파일에 접근할 수 없습니다.\n", DATA_PATH); //참고: '\a'는 윈도우상의 경고음이 들리게 해준다.
        printWaitWindow_MW();
        return;
    }
    fread(&WORKER_LIST, sizeof(WORKER_LIST), 10, dataFile);
    printf("\n[정보 파일 데이터를 로딩하였습니다.\n");
    fclose(dataFile);
...
void fileSave() {
    ...
    FILE* dataFile = NULL;
    if (!(0 == fopen_s(&dataFile, DATA_PATH, "wb+"))) {
        printf("\a\n%s경로의 파일에 작성할 수 없습니다.\n", DATA_PATH); //참고: '\a'는 윈도우상의 경고음이 들리게 해준다.
        printWaitWindow_MW();
        return;
    }
    fwrite(&WORKER_LIST, sizeof(WORKER_LIST), 10, dataFile);
    fclose(dataFile);
}
