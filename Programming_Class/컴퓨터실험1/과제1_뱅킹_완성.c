...
const char* DATA_PATH = "C:\\Users\\bkcha\\Desktop\\data.txt";
...
void fileRead() {
    ...
    while (!feof(dataFile)){
        ...
        long bal = 0;
        long addr = 0;
        fscanf_s(dataFile, "%s %d %ld %ld\n", name, sizeof(name), &pass, &bal, &addr);
        strcpy_s(BANK_ACCOUNT_LIST[count].userName, 11, name);
        BANK_ACCOUNT_LIST[count].userPassword = pass;
        BANK_ACCOUNT_LIST[count].userBalance = bal;
        BANK_ACCOUNT_LIST[count].userAddress = addr;
        count++;
    }
...
void fileSave() {
    ...
    for (int a = 0; a < MAX; a++) {
        if (BANK_ACCOUNT_LIST[a].userPassword > 0) {
            fprintf(dataFile, "%s %d %ld %ld\n", BANK_ACCOUNT_LIST[a].userName, BANK_ACCOUNT_LIST[a].userPassword, BANK_ACCOUNT_LIST[a].userBalance, BANK_ACCOUNT_LIST[a].userAddress);
        }
    }
